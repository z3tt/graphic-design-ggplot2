...
You can download the data we are using during this tutorial here: http://cedricscherer.com/data/chicago-nmmaps.csv (but you don't have to).
...
*Note: The `::` is called __namespace__ and can be used to access a function without loading the package. Here, you could also run `library(readr)` first and `chic <- read_csv(...)` afterwards.*
...
```{r ggplot-default-line-point, fig.cap="Again, the same data, now shown as a connected scatterplot as a combination of a points and lines."}
...
```{r ggplot-default-line_col-size-shape, fig.cap="You can style each geometrical layer on its own. Each geometry also comes with a set of individual properties."}
g + geom_point(color = "firebrick", shape = "diamond", size = 2) +
