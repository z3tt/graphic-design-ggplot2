```{r setup-section-walkthrough, echo=FALSE, include=FALSE}
...
Once the **{ggplot2}** package is loaded, we can create a basic ggplot by specifying the *data*, *aesthetics*---the positional mapping of the variables---, and a *geometry* (Fig. \@ref(fig:ggplotBasic)).

```{r ggplotBasic, fig.cap="A basic scatter plot of feels-like temperature and reported TfL bike rents, created with the {ggplot2} package."}
library(ggplot2)

ggplot(data = bikes) + ## data
  aes(x = temp_feel, y = count) + ## aesthetics
  geom_point() ## geometric layer
```

In most cases, you'll find ggplot code in which the aesthetics are supplied inside the `ggplot()` call; however, both versions are valid.

```{r ggplotBasicAesInside, eval=FALSE}
...
Due to so-called *implicit matching*, we can omit the argument names `data` and `mapping` in the `ggplot()` call as long as we first add the data object and afterwards the `aes()` function. You can read more about how to specify data and aesthetics in Chapter XYZ.


</p>

## Combine multiple layers {#layers}
...
```{r ggplotBasicSmooth, fig.cap="The same scatter plot, now with an additional GAM smoothing."}
...
By looking at these scatter plots, we can actually identify two different  trends for day and night. We can highlight both groups by mapping the `day_night` variable to `color` (Fig. \@ref(fig:ggplotBasicColor)). Note that the mapping is applied to both, `geom_point()` and `geom_smooth()` and consequently the latter creates two separate smoothings.

```{r ggplotBasicColor, fig.cap="The points and smoothing lines colored by the time of the day."}
...
```{r ggplotBasicAdjustLayers, fig.cap="We can map aesthetics and define properties for each layer individually."}
ggplot(bikes, aes(x = temp_feel, y = count)) + 
  geom_point(
    ## color mapping only applied to points
    aes(color = day_night), 
    ## larger points with 50% opacity
    alpha = .5, size = 2
  ) + 
  stat_smooth(
...
The `facet` functionality also comes with the option to overwrite the default behavior by "freeing" the positional scales. Furthermore, one can ensure equal axis spacing by freeing the space as well (Fig. \@ref(fig:ggplotFacetGrid)). This setting allows to efficiently use the available space while ensuring comparability across plots and decreasing the potential of misleading the viewer. The differences between the two ways to create small multiples, namely `facet_grid()` and `facet_wrap()`, as well as ways to adjust and annotate facets are explained in Chapter XYZ.

```{r ggplotFacetGrid, fig.cap="{ggplot2} even allows to free the axis range—while ensuring equal axis spacing."}
ggplot(bikes, aes(x = temp_feel, y = count)) + 
  geom_point(
...
```{r ggplotLabelsAxes, fig.cap="To adjust the formatting of axis labels, the respective axis needs to be addressed via the `scale_*()` functions."}
ggplot(bikes, aes(x = temp_feel, y = count)) + 
  geom_point(
...
## Use a custom color palette {#colors}
...
```{r ggplotLabels, fig.cap="We can overwrite the default labels with the `labs()` which also allows to add a title, subtitle, caption, and tag to the graphic."}
g2 <- g1 +
  labs(
...
```{r legendLabels, fig.cap="By passing a vector of the same length as there are seasons to the `labels` argument of the `scale_color_*()` function, we can overwrite the labels used in the legend."}
g3 <- g2 +
  scale_color_manual(
    values = c("#3c89d9", "#1ec99b", "#F7B01B", "#a26e7c"),
...
```{r ggplotThemeComplete, fig.cap="The graphic now comes with a new look and a non-default font used for all text labels."}
g3 +
...
```{r ggplotThemeAdjustments, fig.cap="After applying on of the complete themes, one can further style the theme to the personal needs."}
g3 +
  theme_light(
    base_size = 12, base_family = "Cabinet Grotesk"
  ) +
