...
```{r class-character, collapse=TRUE}
...
```{r factors-change, collapse=TRUE}
bikes$season_type_mix <- factor(bikes$season, levels = c("2", "0", "3", "1"))
levels(bikes$season_type_mix)
```

The [**{forcats}** package](https://forcats.tidyverse.org/) provides a set of useful functions to reorder factor levels. Important functions to quickly change to order of variables include `fct_rev()`, `fct_reorder()`, `fct_infreq()`, `fct_inorder()`, and `fct_lump()`:

```{r factors-forcats, collapse=TRUE}
...
```{r classes-numerical, collapse=TRUE}
...
```{r coercion, warning=FALSE, collapse=TRUE}
...
```{r classes-datetime, collapse=TRUE}
...
```{r, inspect-data-vars, collapse=TRUE}
...
The [**{dplyr}**](https://dplyr.tidyverse.org/) and other **{tidyverse}** packages provide a simple and intuitive syntax to wrangle data. There are a ton of unique functions, but knowing a handful is enough to empower you to bring your data in the right format. 

There are five main functions of **{dplyr}**, called *verbs* in this context:
...
### Quarto and Rmarkdown {#quarto-rmd}
