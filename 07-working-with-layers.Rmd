...
s1 <- 
  ...
  stat_count() +
  scale_fill_manual(values = prismatic::clr_darken(colors[2:3], .1), name = NULL) +
  labs(title = "geom_bar()\nstat_count()") +
...
While the stacked bars shown in Fig. \@ref(fig:07layerPredefinedDefault) (left) emphasize the total counts per weather type, a grouped bar chart simplifies comparison between groups. We can change the default behavior of `geom_bar()` or `stat_count()` by setting the `position` to `"dodge"` to place the bars next to each other:
...
```{r echo, FALSE, fig.cap='Left: The bar chart of weather types per time of the day as dodged bars by setting the `position` to `"dodge"`, overwriting the default `"stack"`. Right: The conditional smoothing as so-called pointranges displaying the predicted mean as points and the confidence intervals as vertical lines by overwriting the default `geom` in `stat_smooth()` or using the respective geometrical layer with `stat = "smooth"`.'}
s3 <- 
  ggplot(bikes, aes(x = weather_type, fill = day_night)) +
  geom_bar(position = position_dodge2(preserve = "single", width = .9)) +
...
```{r 07layerTypeBoxplot, echo=FALSE, fig.cap='The number of indiviudal box-and-whisker plots created by`geom_boxplot()` depends on the type of variable mapped to x. In case of categorical viarables such as `season` or `is_weekend`, the function creates as many boxplots as there are categories. For numeric variables such as `temp`, only a single boxplot isccalculated by default.'}
...
We can force **ggplot2** to grouped the quantitative variable into equal numerical ranges, so-called *bins* by wrapping the variable inside `cut_interval()`. The corrresponding `cut_number()` creates groups with equal number of observations (and potentially unequal ranges).
...
```{r 07layerPositionalTypeBoxplotCut, echo=FALSE, fig.cap=''}
...
```{r 07layerPositionalTypeHistogram, echo=FALSE, fig.cap='A histogram bins a quantitative variable, showing counts of obersvations per group (left). As qualitative data is already grouped, a regular bar chart is used to display counts per category (right).'}
