...
## apply to the `mpg` daat from {ggplot2}
...
```{r, echo=FALSE, purl=FALSE, fig.height=5, fig.cap='Two example use cases of the custom `plot_density()` function applied to three variables of the `mpg` data (upper row) and the `bikes` data (lower row), respectively. For the latter, `day_night` was passed as grouping variable.'}
plot_density <- function(data, var, grp = "") {
  g <- 
    ggplot(data, aes(x = !!sym(var))) +
    geom_density(aes(fill = !!sym(grp)), position = "identity",
